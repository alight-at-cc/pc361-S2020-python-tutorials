...
Slight modfifications by Eric Jensen, ejensen1@swarthmore.edu, to be
compatible with both Python 2 and Python 3.  February 2016.
...
def read_hdf5(filename, readstrain=True):
    ...
    # Somewhat ugly hack to convert the lists of things that
    # will be turned into dictionary keys into regular
    # strings that will work with both Python 2 and Python 3
    # without having to specify as b'KEYNAME' in Python 3:
    import sys
    if sys.version_info >= (3,):
        shortnameList = [x.decode("utf-8") for x in shortnameList]
        injnameList = [x.decode("utf-8") for x in injnameList]
